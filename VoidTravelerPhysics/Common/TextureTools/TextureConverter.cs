...
namespace FarseerPhysics.Common
{
	...
	public class DetectedVertices : Vertices
	{
		...
		internal void Transform(Matrix4x4 transform)
		{
			...
			if (this._holes != null && this._holes.Count > 0)
			{
				...
				{
					temp = this._holes[i].ToArray();
					for (int k = 0; k < temp.Length; k++)
					{
						temp[k].X = ((temp[k].X * transform.M11) + (temp[k].Y * transform.M21)) + transform.M41;
						temp[k].Y = ((temp[k].X * transform.M12) + (temp[k].Y * transform.M22)) + transform.M42;
					}

					this._holes[i] = new Vertices(temp);
				}
			}
		}
	}
	...
	public sealed class TextureConverter
	{
		...
		{
			get { return this._polygonDetectionType; }
		...
		{
			get { return this._holeDetection; }
		...
		{
			get { return this._multipartDetection; }
		...
		{
			get { return this._pixelOffsetOptimization; }
		...
		{
			get { return (byte)(this._alphaTolerance >> 24); }
		...
		public List<DetectedVertices> DetectVertices()
		{
			...
			// Post processing.
			if (this.PolygonDetectionType == VerticesDetectionType.Separated) // Only when VerticesDetectionType.Separated? -> Recheck.
			...
			if (this._transform != Matrix4x4.Identity)
		...
		{
			for (int i = 0; i < detectedPolygons.Count; i++)
				detectedPolygons[i].Transform(this._transform);
		}
		...
		public bool IsSolid(ref Vector2 v)
		{
			...
			if (this._tempIsSolidX >= 0 && this._tempIsSolidX < this._width && this._tempIsSolidY >= 0 && this._tempIsSolidY < this._height)
				return (this._data[this._tempIsSolidX + this._tempIsSolidY * this._width] >= this._alphaTolerance);
		...
		public bool IsSolid(ref int x, ref int y)
		{
			if (x >= 0 && x < this._width && y >= 0 && y < this._height)
				return (this._data[x + y * this._width] >= this._alphaTolerance);
		...
		public bool IsSolid(ref int index)
		{
			if (index >= 0 && index < this._dataLength)
				return (this._data[index] >= this._alphaTolerance);
		...
		private bool DistanceToHullAcceptable(Vertices polygon, Vector2 point, bool higherDetail)
		{
			...
			if (higherDetail)
			{
				for (int i = 0; i < polygon.Count; i++)
				{
					...
					if (LineTools.DistanceBetweenPointAndLineSegment(ref point, ref edgeVertex1, ref edgeVertex2) <= this._hullTolerance ||
						LineTools.DistanceBetweenPointAndPoint(ref point, ref edgeVertex1) <= this._hullTolerance)
					{
						return false;
					}
		...
		private bool SearchNextHullEntrance(List<DetectedVertices> detectedPolygons, Vector2 start, out Vector2? entrance)
		{
			...
			for (int i = (int)start.X + (int)start.Y * this._width; i <= this._dataLength; i++)
			{
				if (IsSolid(ref i))
				{
					if (foundTransparent)
		...
		private bool SearchForOutstandingVertex(Vertices hullArea, out Vector2 outstanding)
		{
			...
			if (hullArea.Count > 2)
			{
				...
				for (int i = 1; i < hullAreaLastPoint; i++)
				{
					...
					if (LineTools.DistanceBetweenPointAndLineSegment(ref tempVector1, ref tempVector2, ref tempVector3) >= this._hullTolerance)
					{
